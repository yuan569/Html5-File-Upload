...
(function() {
    ...
    FileUpload.prototype = {
            ...
            while (i < len) {
                ...
                (function(j) {
                    ...
                    img.onload = function() {
                        ...
                        //点击按钮新增图片项目（创建DOM结构并插入到上传按钮元素之前）,并将keyName的值传给参数id，id用来标记图片所属的项目在列表中的位置
                        self.addItem(compressedDataURL);
        ...
        //点击按钮新增图片项目
        addItem(url) {
            //创建DOM结构，并插入到上传按钮元素之前
            var domFrag = this.createDom(url);
            this.uploadFlexbox.insertBefore(domFrag, this.btnBelongToFlexItem);
        },
        //点击按钮删除图片项目
        deleteItem(target) {
            var self = this,
        ...
        recordFileName: function(data) {
            ...
            var j = itemsLen - filesLen

            //为节点项目标记文件名
            fileNames.forEach(function(item, index) {
                items.item(j + index - 1).setAttribute("fname", item.fname)
            })
        },
